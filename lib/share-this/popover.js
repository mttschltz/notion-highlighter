...
export function popoverClick(sharers, event, range) {
    ...
    if (sharer && isCallable(sharer.action)) {
        sharer.action(event, item, range);
    }
}
...
export function lifeCycleFactory(document) {
    return {
        ...
        attachPopover(popover, range) {
            const clonedRange = range.cloneRange(); // not sure if required
            document.body.appendChild(popover);
            popover.addEventListener("click", function(event) {
                // Sometimes the range here has changed and the toString() value is empty. Not sure why. Might be related to the click event on the popover clearing the selection.
                // If that happens, might need to create toString() value outside of this listener, when it's still the desired range, and then use that value here in the listener.
                popoverClick(this.sharers, event, clonedRange);
            });
        },
